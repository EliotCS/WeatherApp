...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if (savedLat && savedLon) {
        ...
        getWeatherByCoords(savedLat, savedLon);
    } 
    
    else if (savedState && savedCity) {
...
async function getWeatherData(fstate,fcity) {
    try{
        ...
        localStorage.setItem("CityInput", fcity);
        localStorage.setItem("StateInput", fstate);
...
async function getWeatherByCoords(lat, lon) {
    try {
        ...
        const fullStateName = geoData[0]?.state || "";
        const state = getStateAbbreviation(fullStateName);
        ...
        localStorage.setItem("savedLat", lat);
        localStorage.setItem("savedLon", lon);
        localStorage.setItem("CityInput", city || WeatherAPI.city.name);
        localStorage.setItem("StateInput", state);

        const DisplayCurrentLoco = document.getElementById("DisplayLoco");
        DisplayCurrentLoco.textContent = state ? `${city || WeatherAPI.city.name}, ${state}` : `${WeatherAPI.city.name}, ${WeatherAPI.city.country}`;
        CreateWeatherCards(WeatherAPI);
    } 
    catch (error) {
...
const usStateAbbreviations = {"Alabama": "AL", "Alaska": "AK", "Arizona": "AZ", "Arkansas": "AR", "California": "CA",
...
function getStateAbbreviation(stateName) {
    if (!stateName) {
        return "";}
    return usStateAbbreviations[stateName] || stateName;
}
